...
var familyContracts = map[string]FamilyContract{
	...
	"persistence-path": {
		...
		Status:         StatusImplemented,
		Meaning:        "A current identity can already preserve Azure access through a durable cloud-side object, or it is one defended durable object away from doing so.",
		Summary:        "Follow the strongest cloud-side persistence stories toward the durable identity or trigger object that would preserve access later.",
		AllowedClaim:   "Can claim durable cloud persistence only when HO-Azure can name the durable identity or trigger object and the current-identity control path behind it. Cannot claim malicious content, exploit execution, or serverless persistence from inventory alone.",
		CurrentGap:     "Phase 1 stays intentionally strict: identity shadowing and Automation dominate the admitted rows today, while Function, App Service, Logic App, Event Grid, ACR, and DevOps persistence stories wait for clearer trigger or durable-object proof so the family does not drift into execution-only or inventory language.",
		BestCurrentExamples: []string{
			"app-credentials -> persistence-path",
			"automation + rbac -> persistence-path",
		},
		BackingCommands:    []string{"app-credentials", "automation", "permissions", "rbac", "functions", "app-services", "managed-identities", "role-trusts"},
		PreferredArtifacts: []string{"loot", "json"},
		StructuredRowFields: []string{
			"surface",
			"persistence_type",
		...
		OperatorQuestion: "Which Azure-native service or identity surface lets the current identity establish durable cloud persistence, or the near-complete conditions to set it up?",
		SourceCommandMinimum: []FamilySourceContract{
			{Command: "app-credentials", MinimumFields: []string{"row_class", "target_object_type", "target_object_id", "target_object_name", "credential_type", "control_path", "current_evidence", "missing_proof", "operator_actionability", "recommended_fix_focus"}, Rationale: "Provides the strongest current identity-shadowing and durable trust evidence in phase 1."},
			{Command: "automation", MinimumFields: []string{"id", "name", "published_runbook_count", "schedule_count", "job_schedule_count", "webhook_count", "primary_runbook_name", "identity_join_ids", "missing_target_mapping"}, Rationale: "Provides the durable trigger-backed execution surfaces that can honestly anchor Azure-native persistence."},
			{Command: "permissions", MinimumFields: []string{"principal_id", "display_name", "high_impact_roles", "scope_ids", "privileged", "is_current_identity"}, Rationale: "Provides severity and current-identity anchors for persistence rows and the identities they would preserve."},
			{Command: "rbac", MinimumFields: []string{"principal_id", "role_name", "scope_id"}, Rationale: "Provides the edit-capable scope proof needed before Automation rows can claim the current identity can establish or reuse persistence here."},
			{Command: "functions", MinimumFields: []string{"id", "name", "default_hostname", "azure_webjobs_storage_value_type", "summary"}, Rationale: "Provides the current serverless visibility boundary so Function rows stay out of strong persistence wording until durable trigger proof is visible."},
			{Command: "app-services", MinimumFields: []string{"id", "name", "default_hostname", "workload_identity_ids", "summary"}, Rationale: "Provides the current App Service visibility boundary so generic web-app inventory does not become fake persistence."},
			{Command: "managed-identities", MinimumFields: []string{"id", "name", "principal_id", "attached_to", "scope_ids"}, Rationale: "Provides supporting identity context when durable persistence preserves access behind an attached managed identity."},
			{Command: "role-trusts", MinimumFields: []string{"trust_type", "source_object_id", "target_object_id", "confidence", "summary"}, Rationale: "Provides supporting identity-control context when a persistence row depends on visible ownership or trust expansion boundaries already modeled elsewhere."},
		},
	},
}
